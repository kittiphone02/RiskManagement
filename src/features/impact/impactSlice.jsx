...
export const getRiskImpacts = createAsyncThunk(
  "impacts/getRiskImpacts",
  async (_, { dispatch }) => {
...
export const addImpact = createAsyncThunk(
  "impacts/addImpact",
  async (body, { dispatch }) => {
    dispatch(showLoading());
    try {
...
export const updateImpact = createAsyncThunk(
...
export const deleteImpact = createAsyncThunk(
  "impacts/deleteImpact",
  async (id, { dispatch }) => {
    dispatch(showLoading());
    try {
      await axios.delete(`${API_URI}/api/impacts/${id}`);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateImpact.fulfilled, (state, action) => {
        const index = state.impacts.findIndex(
          (impact) => impact._id === action.payload.data._id
        );
        if (index !== -1) {
