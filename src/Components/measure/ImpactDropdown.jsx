...
import { getRiskImpacts } from "../../features/impact/impactSlice";

function ImpactDropdown({ value, setValue }) {
  ...
  return (
    <Listbox value={value} onChange={(val) => setValue(val)}>
      <div className="relative mt-1">
        <Listbox.Button
          className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2
                     text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
        >
          <div className="flex items-center">
            <span className="ml-3 block truncate font-lao">
              {value ? value.value : "ເລືອກຜົນກະທົບ"}
            </span>
          </div>
